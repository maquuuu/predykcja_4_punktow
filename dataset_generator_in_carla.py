...
IMAGE_H = 2160
DEFAULT_FOV = 90.0
OUTPUT_DIR = "dataset_autopilot_witold"
...
CAMERA_MONITORING_VIEW_PROB = 0.65

IGNORE_OCCLUDED_KEYPOINTS_FOR_POSE = True
...
MIN_DETECTION_VISIBLE_POINTS = 2


POSE_MIN_BOX_WIDTH_PX = 180
POSE_MIN_BOX_HEIGHT_PX = 120
POSE_MIN_BOX_AREA_PX = POSE_MIN_BOX_WIDTH_PX * POSE_MIN_BOX_HEIGHT_PX
POSE_MAX_TRUNCATION_RATIO = 0.25
MIN_POSE_VISIBLE_POINTS = 2
...
def main():
    ...
    try:
        ...
        print("Static parked vehicles, foliage and street lights removed.")
    except RuntimeError:
        pass
    ...
    target_vehicle = spawn_random_car(world, car_blueprints, waypoints)
    start_transform = target_vehicle.get_transform()
    print(f"Target vehicle: {target_vehicle.type_id}")
    ...
    try:
        while counter <= target_limit:
            ...
            if (counter - 1) % 10 == 0 and counter > 1:
                ...
                target_vehicle = spawn_random_car(world, car_blueprints, waypoints)
                print(f"Target vehicle changed to: {target_vehicle.type_id}")
            ...
            if random.random() < CAMERA_MONITORING_VIEW_PROB:
            ...
            for _ in range(10):
                ...
                if len(ray_trace) > 0:
                    ...
                    if hit_label not in valid_vehicle_labels:
                        continue

                target_look_location = carla.Location(
                    x=target_transform.location.x + random.uniform(-4.0, 4.0),
                    y=target_transform.location.y + random.uniform(-4.0, 4.0),
                    z=target_transform.location.z + random.uniform(0.5, 1.5),
                )
            ...
            random_fov = random.uniform(75.0, 100.0)
            ...
            try:
                ...
                print(f"Saved image {counter}/{target_limit} with {len(label_lines)} labels.")
                counter += 1
